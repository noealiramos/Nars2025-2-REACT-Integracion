...
import { createProduct, getProducts, getProductById, updateProduct, deleteProduct, searchProducts } from '../../../src/controllers/productController.js';
...
describe('productController', () => {
    ...
    describe('getProductById', () => {
        it('debe retornar 404 si el producto no existe', async () => {
            const { req, res, next } = createMockReqRes({ params: { id: 'p1' } });
            Product.findById.mockReturnValue({ populate: vi.fn().mockReturnThis(), lean: vi.fn().mockResolvedValue(null) });
    ...
    describe('updateProduct', () => {
        it('debe actualizar el producto y retornar 200', async () => {
            ...
            const mockUpdateQuery = { populate: vi.fn().mockResolvedValue({ _id: 'p1', name: 'Updated' }) };
            Product.findByIdAndUpdate.mockReturnValue(mockUpdateQuery);
    ...
    describe('deleteProduct', () => {
        it('debe eliminar el producto (200)', async () => {
            const { req, res, next } = createMockReqRes({ params: { id: 'p1' } });
    ...
    describe('searchProducts', () => {
        it('debe buscar productos usando regex (200)', async () => {
            const { req, res, next } = createMockReqRes({ query: { q: 'oro' } });
            Product.countDocuments.mockResolvedValue(1);
            const mockQuery = {
                populate: vi.fn().mockReturnThis(),
                skip: vi.fn().mockReturnThis(),
                limit: vi.fn().mockReturnThis(),
                lean: vi.fn().mockResolvedValue([{ name: 'Anillo Oro' }])
            };
            vi.mocked(Product.find).mockReturnValue(mockQuery);
