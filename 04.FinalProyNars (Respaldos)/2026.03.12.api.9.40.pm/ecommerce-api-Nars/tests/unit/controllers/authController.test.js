...
describe('authController', () => {
    ...
    describe('refresh', () => {
        it('debe rotar el refresh token exitosamente', async () => {
            const { req, res, next } = createMockReqRes({
            ...
            User.findById.mockResolvedValue({ _id: 'u1', active: true, displayName: 'Test', role: 'customer' });
    ...
    describe('logout', () => {
        it('debe invalidar el token de refresco al cerrar sesión', async () => {
            const { req, res, next } = createMockReqRes({
                body: { refreshToken: 'mock_refresh' }
            });
