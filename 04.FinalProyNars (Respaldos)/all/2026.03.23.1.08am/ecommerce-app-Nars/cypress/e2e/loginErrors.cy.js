describe('Flujos Adversos - Autenticación', () => {
  ...
  it('Debería manejar correctamente el inicio de sesión con credenciales inválidas', () => {
    ...
    cy.get('[data-testid="input-email"]').clear().type('invitado.falso@mail.com')
    cy.get('[data-testid="input-password"]').clear().type('11111111') // Pass erróneo
    ...
    cy.wait('@loginReqFailed').its('response.statusCode').should('be.oneOf', [400, 401])
