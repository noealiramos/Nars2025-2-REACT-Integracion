...
describe('shippingAddressController', () => {
    ...
    describe('getAddressById', () => {
        it('debe retornar 404 si la dirección no existe para ese usuario', async () => {
            const { req, res, next } = createMockReqRes({
                params: { addressId: 'a1' },
                user: { id: 'u1' }
            });
    ...
    describe('deleteShippingAddress', () => {
        it('debe eliminar la dirección si pertenece al usuario (200)', async () => {
            const { req, res, next } = createMockReqRes({
                params: { addressId: 'a1' },
                user: { id: 'u1' }
            });
            ShippingAddress.findOne.mockReturnValue({ lean: vi.fn().mockResolvedValue({ _id: 'a1' }) });
